...
// @desc    Register/Create new user with auto-generated password
// @route   POST /api/auth/register
// @access  Private (Superadmin/Team Lead)
exports.register = async (req, res, next) => {
  try {
    ...
    // Validate phone number for password generation
    if (!phone || phone.length < 4) {
      return error(res, 'Valid phone number is required (at least 4 digits)', 400);
    }

    // Generate default password
    const defaultPassword = generateDefaultPassword(name, phone);
    ...
    await createNotification({
      ...
      type: 'user_created',
      title: 'Welcome to Task Manager',
      message: `Your account has been created by ${req.user.name}. Your default password is: ${defaultPassword}`,
      referenceId: user._id,
      referenceType: 'user'
    });
    ...
    return success(res, {
      ...
      defaultPassword: defaultPassword // Send to admin to inform user
    }, 'User created successfully. Default password generated.', 201);
  } catch (err) {
    next(err);
  }
};
